...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x {0} "cd {1}; nice {2} -c {3} -g {4} -v {5} {6} {7}"'.format \
          (self.host,self.cwd, \
           svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
...
class SGEWorker(Worker):
    def __init__(self,name,job_queue,result_queue):
        Worker.__init__(self,name,job_queue,result_queue)
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'qrsh -l h_rt=01:00:00 -l h_vmem=2500M -now no -N {0} "cd {1}; {2} -c {3} -g {4} -v {5} {6} {7}"'.format \
          (self.name,self.cwd, \
           svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        print '%s started job ' % self.name
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
	    if str(line).find("Cross") != -1:
		print line
...
def main():
    ...
    if nr_sge_worker:
        for i in range(nr_sge_worker):
            SGEWorker('worker_'+str(i),job_queue,result_queue).start()
            if i%2 == 0:
		time.sleep(1)
