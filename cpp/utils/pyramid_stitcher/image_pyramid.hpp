...
public:
	...
	ImagePyramid(int padx, int pady, int interval, const std::vector<Level> & levels);
	...
	/// @note The amount of padding and the interval should be at least 1.
	ImagePyramid(const cv::Mat & image, int padx, int pady, int interval = 10, int upsampleFactor = 2);
	...
	/// @param[in] padx Amount of horizontal zero padding (in cells).
	/// @param[in] pady Amount of vertical zero padding (in cells).
	/// @param[in] numScales Number of scale in pyramid.
  /// @param[in] scaleFactor Scale factor to downscaling of each scale
  /// @param[in] upsampleFactor = how big should top scale be? (compared to input img)
  /// @param[in] mean pixel to fill the borders after padding
	/// @note The amount of padding and the interval should be at least 1.
	ImagePyramid(const cv::Mat & image, int padx, int pady, int numScales, float scaleFactor = 0.85, int upsampleFactor = 1, cv::Scalar mean = cv::Scalar(0,0,0));
    ...
    void AvgLerpPad(cv::Mat & image, cv::Scalar mean);
