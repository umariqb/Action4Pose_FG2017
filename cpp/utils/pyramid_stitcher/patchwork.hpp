...
public:
	...
	Patchwork(ImagePyramid & pyramid, cv::Scalar mean = cv::Scalar(0,0,0));

    //remove scales that don't fit in planes
    void prune_big_scales(ImagePyramid & pyramid);

    //remove scales that are smaller than the template
    void prune_small_scales(ImagePyramid & pyramid, int img_minWidth_, int img_minHeight_);
