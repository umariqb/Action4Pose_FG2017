...
float PixelValueFeatuers::extract(const std::vector<cv::Mat>& images,
                                const cv::Rect& rect,
  ...
  return images[featureChannel].at<float>(
...
void PixelValueFeatuers::generate(int width,
  ...
  boost::uniform_int<> dist_size_x(0, width-1);
  RandGen rand_x(*rng, dist_size_x);
  boost::uniform_int<> dist_size_y(0, height-1);
...
float PatchComparisionFeature::extract(const std::vector<cv::Mat>& featureChannels,
																const cv::Rect& rect,
...
float PatchComparisionFeature::extract(const std::vector<cv::Mat>& images,
																const cv::Rect& rect,
																bool is_integral_image) const {
...
float PixelComparisonFeature::extract(const std::vector<cv::Mat>& images,
																const cv::Rect& rect,
																bool is_integral_image) const {
	CHECK(!is_integral_image);
	...
	float a = images[featureChannel_a].at<float>(
			cv::Point(rect.x + point_a.x, rect.y + point_a.y));
	float b = images[featureChannel_b].at<float>(
