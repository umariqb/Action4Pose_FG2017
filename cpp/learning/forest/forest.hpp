...
public:
  ...
  //stores the tree
  void save(std::string url, int offset = 0) const {
    for (unsigned int i = 0; i < trees.size(); i++) {
      char buffer[200];
      sprintf(buffer, "%s%03d.txt", url.c_str(), i + offset);

      std::string path = buffer;
      CHECK(Tree<Sample>::save_tree(path, trees[i]));
    }
  }
	...
	static bool load_forest(std::string config_file,
		...
		if(loadConfigFile(config_file, param)) {
			return load_forest(param, forest, num_trees);
		}
		return false;
	}
  ...
  static bool load_forest(ForestParam param,
                          Forest<Sample>* forest,
                          int num_trees = -1) {
    forest->load(param.tree_path, param, num_trees);
    return forest->trees.size() > 0;
  }

  bool load(std::string url, ForestParam tp, int max_trees = -1) {
  ...
  static bool load_tree(std::string url, std::vector<Tree<Sample>*>& trees) {
    ...
    Tree<Sample>* tree;
    if( Tree<Sample>::load_tree(url,&tree)) {

      if (tree->isFinished()) {
        trees.push_back(tree);
      } else {
       // delete tree;
        LOG(INFO)<<"Tree is unfinishsed: "<<url;
        return false;
      }
      return true;
    }
