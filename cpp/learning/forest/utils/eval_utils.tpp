...
//  std::vector<cv::Mat> feat_channels = sample.get_feature_channels();

	for (int x = roi.x; x < roi.x + roi.width; x += step_size) {
		for (int y = roi.y; y < roi.y + roi.height; y += step_size) {


      cv::Point_<int> center(x, y);

//      cv::Mat feature = cv::Mat::zeros(1,feat_channels.size(), CV_32F);
//      for(unsigned int fIdx=0; fIdx<feat_channels.size(); fIdx++){
//        feature.at<float>(0,fIdx) = feat_channels[fIdx].at<float>(center);
//      }

 //     Image patch_sample;
//      patch_sample.add_feature_channel(feature);

      cv::Rect patch_bbox(x, y, 1, 1);
      T s(&sample, patch_bbox);
  ...
  if(blur){
   cv::GaussianBlur(voting_map, voting_map, cv::Size(), 3, 3, cv::BORDER_REFLECT);
  }
}
...
//	}
  ...
  std::vector<cv::Mat> feat_channels = sample.get_feature_channels();
  std::cout<<feat_channels.size()<<std::endl;
	for (int x = roi.x; x < roi.x + roi.width; x += step_size) {
		for (int y = roi.y; y < roi.y + roi.height; y += step_size) {

		  sliding_window.x = x;
      sliding_window.y = y;
      cv::Point_<int> loc(x,y);

//      cv::Mat feature = cv::Mat::zeros(1,feat_channels.size(), CV_32F);
//      for(unsigned int fIdx=0; fIdx<feat_channels.size(); fIdx++){
//      feature.at<float>(0,fIdx) = feat_channels[fIdx].at<float>(loc);
//      }
...
//      patch_sample.add_feature_channel(feature);
      ...
      T s(&sample, patch_bbox);
			...
			foreground_map.at<float> (loc) = foreground/leafs.size();
		}
	}
