...
// int->float
typedef std::pair<float, unsigned int> IntIndex;
struct less_than {
...
public:
  ...
  template<typename Sample>
  float get_feature_value(const Sample* sample, const Feature* feature) const {
    return feature->extract(sample->get_images(),
                               sample->get_roi(),
                               sample->is_integral_image());
  }
//  vision::features::PixelComparisonFeature::extract()
  template<typename Sample>
