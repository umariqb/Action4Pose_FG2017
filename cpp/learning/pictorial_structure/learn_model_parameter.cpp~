...
//    }
  ...
  void get_body_model_mixtures(std::string index_path,
    ...
    vector<Annotation> org_annotations, annotations, rot_annotations;
    ...
    bool use_rotated_annotations = false;
    if(use_rotated_annotations){
      LOG(INFO)<<"Adding rotated annotations.";
      vector<double> rot_angles;
      rot_angles += -75, -60,-45,-37.5,-30,-22.5,-15,-7.5,7.5,15, 22.5, 30, 37.5, 45, 60,75;

      for(unsigned int i=0; i<annotations.size(); i++){
        Annotation ann = annotations[i];
        cv::Mat image = cv::imread(ann.url);

        for(unsigned int a=0; a<rot_angles.size(); a++){
          cv::Mat rot_image;
          Annotation rot_ann;
          rotate_image(image, ann, cv::Point(image.cols/2, image.rows/2), rot_angles[a], rot_image,  rot_ann);
          rot_annotations.push_back(rot_ann);
//          plot(rot_image, rot_ann.parts, pose_type);
//          LOG(INFO)<<::utils::VectorToString(rot_ann.parts);
        }
      }
    }
    annotations.insert(annotations.end(), rot_annotations.begin(), rot_annotations.end());
...
//    for(int i=0; i<ann_cleaned.size(); i++){
//      if(class_hist.size() <= ann_cleaned[i].cluster_id){
//        class_hist.resize(ann_cleaned[i].cluster_id+1, 0);
//      }
//      class_hist[ann_cleaned[i].cluster_id]++;
//    }
//    LOG(INFO)<<"Class Hist: "<<::utils::VectorToString(class_hist);
//
